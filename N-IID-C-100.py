...
def get_cifar100():
  ...
  data_train = torchvision.datasets.CIFAR100('./data', train=True, download=True) 
  
  x_train, y_train = data_train.data.transpose((0,3,1,2)), np.array(data_train.targets)
  
  return x_train, y_train

def print_image_data_stats(data_train, labels_train):
  print("\nData: ")
...
def get_default_data_transforms(train=True, verbose=True):
  ...
  return (transforms_train['cifar100'])

def get_data_loaders(nclients,batch_size):
  
  x_train, y_train = get_cifar100()


  print_image_data_stats(x_train, y_train)
...
np.save('N-IID_CIFAR100_1000C_2500R_5E_acc_2Run',acc_test)
np.save('N-IID_CIFAR100_1000C_2500R_5E_Loss_2Run',loss_train)